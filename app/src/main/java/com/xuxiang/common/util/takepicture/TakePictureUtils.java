...
import com.xuxiang.common.BuildConfig;
...
public class TakePictureUtils {
    ...
    //第三方获取图片的回调码 (目前是知乎)
    private static final int CODE_ZHIFU_PHOTO = 104;
    //是否在activity中使用
    ...
    public void setTakeCallBacklistener(TakePictureListener takeCallBacklistener) {
     ...
     * 调用第三方的 图片选择（知乎）
     */
    public void getByMatisse() {
        Matisse from = null;
        if (isActicity) {
            from = Matisse.from(mActivity);
        } else {
            from = Matisse.from(mFragment);
        }
        from.choose(EnumSet.of(MimeType.JPEG, MimeType.PNG))
                .countable(false)
                .maxSelectable(1)
                .capture(false)
    ...
    public void attachToActivityForResult(int requestCode, int resultCode, Intent data) {
        ...
        switch (requestCode) {
            ...
            case CODE_ORIGINAL_PHOTO_CAMERA:
                Log.e("takePhotoPath", "" + takePhotoPath);
            ...
            case CODE_ORIGINAL_PHOTO_ALBUM:
                if (data != null) {
                    ...
                    String pickPath = FileUtils.getRealFilePath(mContext, sourceUri);
            ...
            case CODE_TAILOR_PHOTO:
                if (data != null) {
                    if (curUri != null) {
                        //裁剪后再压缩
                        File temFile = new File(FileUtils.getRealFilePath(mContext, curUri));
